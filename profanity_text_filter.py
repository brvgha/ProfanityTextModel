...
labeled_data_df = labeled_data_df.rename(columns={"tweet": "text", "class": "severity_rating"})
...
tfidf = sk.feature_extraction.text.TfidfVectorizer(max_features=len(df))
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(1) # this does probability (0 AND 1)
])
...
model.predict(tfidf.transform(["home"]).toarray())
model.predict(tfidf.transform(["fuck"]).toarray())
